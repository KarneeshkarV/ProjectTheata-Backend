...
func (s *service) HandleCallback(w http.ResponseWriter, r *http.Request) {
	...
	if code == "" {
		...
		http.Redirect(w, r, fmt.Sprintf("%s/app?success=false&error=%s", config.AppConfig.FrontendURL, errMsg), http.StatusTemporaryRedirect)
		return
	}
	...
	if err != nil {
		...
		http.Redirect(w, r, fmt.Sprintf("%s/app?success=false&error=token_exchange_failed", config.AppConfig.FrontendURL), http.StatusTemporaryRedirect)
		return
	}
	...
	if err != nil {
		...
		http.Redirect(w, r, fmt.Sprintf("%s/app?success=false&error=token_encryption_failed", config.AppConfig.FrontendURL), http.StatusTemporaryRedirect)
		return
	}
	...
	if err != nil {
		...
		http.Redirect(w, r, fmt.Sprintf("%s/app?success=false&error=db_save_failed", config.AppConfig.FrontendURL), http.StatusTemporaryRedirect)
		return
	}
